...
export async function POST(request: Request) {
    try {
        ...
        console.log(`[Analytics Proxy] Backend response status: ${response.status}`);

        if (!response.ok) {
            const errorText = await response.text();
            console.error(`[Analytics Proxy] Backend error: ${errorText}`);
            return NextResponse.json({ success: false, error: 'Backend tracking failed' }, { status: response.status });
        }
